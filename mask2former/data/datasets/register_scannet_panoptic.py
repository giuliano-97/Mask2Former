...
def get_scannet_frames_25k_panoptic_dataset_items(
    ...
    for pano_seg_annotation in sorted(annotations_list, key=lambda k: k["image_id"]):
        ...
        # Convert category id
        segments_info = [
            _convert_category_id(s, metadata) for s in pano_seg_annotation["segments_info"]
        ]

        dataset_items.append({
            "image_id": image_id,
            "file_name": str(scannet_frames_25k_dir_path.joinpath(image_info["file_name"]).with_suffix('.jpg')),
            "height": image_info["height"],
            "width": image_info["width"],
            "pan_seg_file_name": str(scannet_panoptic_dir_path / pano_seg_annotation["file_name"]),
...
def get_scannet_frames_25k_metadata(category_info_list):
    ...
    thing_classes = [ci["name"] if ci["isthing"] ==
                     True else "" for ci in category_info_list]
    thing_colors = [ci["color"] if ci["isthing"] ==
                    True else [0, 0, 0] for ci in category_info_list]
...
def register_scannet_frames_25k_panoptic(scannet_frames_25k_dir_path):
    assert scannet_frames_25k_dir_path.is_dir()
    ...
    scannet_panoptic_json_file_path = scannet_frames_25k_dir_path / "scannet_panoptic.json"
    ...
    metadata = get_scannet_frames_25k_metadata(
        scannet_panoptic_info["categories"])

    for split, scans in [('train', scannetv2_train_scans), ('val', scannetv2_val_scans)]:
...
_SCANNET_PANOPTIC_SPLITS = {
    "scannet_frames_25k_train": (
        "scannetv2_train.txt"
    ),
    "scannet_frames_25k_val": (
        "scannetv2_val.txt"
    ),
}

_root = os.getenv("DETECTRON2_DATASETS", "datasets")
register_scannet_frames_25k_panoptic(
    Path("/cluster/scratch/albanesg/scannet_frames_25k"))
